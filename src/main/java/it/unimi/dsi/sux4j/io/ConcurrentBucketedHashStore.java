...
/**
 * Modified from BucketedHashStore to support concurrent add and temp-store-file compression.
 *
 ...
 * {@linkplain #ConcurrentBucketedHashStore(TransformationStrategy, File, int, ProgressLogger) construction
 ...
 * a {@link DuplicateException}. At that point, the caller must handle the
 ...
 * <p>
 * If you specify so
 * {@linkplain #ConcurrentBucketedHashStore(TransformationStrategy, File, int, ProgressLogger) at construction
...
public class ConcurrentBucketedHashStore<T> implements Serializable, SafelyCloseable, Iterable<ConcurrentBucketedHashStore.Bucket> {
    public static final long serialVersionUID = 1L;
    private static final Logger LOGGER = LoggerFactory.getLogger(ConcurrentBucketedHashStore.class);
     ...
     * one) to avoid that calls to {@link ConcurrentBucketedHashStore#checkAndRetry(Iterable)} on a newly created
     ...
     */
    public ConcurrentBucketedHashStore(final TransformationStrategy<? super T> transform) throws IOException {
     ...
     */
    public ConcurrentBucketedHashStore(final TransformationStrategy<? super T> transform, final File tempDir) throws IOException {
     ...
     */
    public ConcurrentBucketedHashStore(final TransformationStrategy<? super T> transform, final ProgressLogger pl) throws IOException {
    ...
    public ConcurrentBucketedHashStore(final TransformationStrategy<? super T> transform, final File tempDir, final ProgressLogger pl) throws IOException {
    ...
    public ConcurrentBucketedHashStore(final TransformationStrategy<? super T> transform, final File tempDir, final int hashWidthOrCountValues, final ProgressLogger pl) throws IOException {
        ...
        for(int i = 0; i < DISK_SEGMENTS; i++) {
            ...
            writableByteChannel[i] = new FileOutputStream(file[i] = File.createTempFile(ConcurrentBucketedHashStore.class.getSimpleName(), String.valueOf(i), tempDir)).getChannel();
            file[i].deleteOnExit();
    ...
    private static void flush(final ByteBuffer buffer, final WritableByteChannel channel) throws IOException {
        buffer.flip();
        ByteBuffer compressed = getCompressor().compress(buffer);
        ByteBuffer header = getHeaderBuffer();
        //System.err.println("compressed to " + compressed.limit());
        header.putInt(compressed.limit());
        header.flip();
    ...
    static ThreadLocal<ZstdCompressCtx> compressor = new ThreadLocal<>();
    private static ZstdCompressCtx getCompressor(){
        ZstdCompressCtx comp = compressor.get();
        if(comp == null){
            comp = new ZstdCompressCtx();
            compressor.set(comp);
        }
        return comp;
    }
    static ThreadLocal<ZstdDecompressCtx> decompressor = new ThreadLocal<>();
    private static ZstdDecompressCtx getDecompressor(){
        ZstdDecompressCtx comp = decompressor.get();
        if(comp == null){
            comp = new ZstdDecompressCtx();
            decompressor.set(comp);
        }
        return comp;
    }

    static ThreadLocal<ByteBuffer> headerBuf = new ThreadLocal<>();
    private static ByteBuffer getHeaderBuffer(){
        ByteBuffer comp = headerBuf.get();
        if(comp == null){
            comp = ByteBuffer.allocateDirect(4);
            headerBuf.set(comp);
        }
        comp.rewind();
        return comp;
    ...
    private static void flush_orig(final ByteBuffer buffer, final WritableByteChannel channel) throws IOException {
        buffer.flip();
        channel.write(buffer);
        buffer.clear();
    }

    private static long readLong_orig(final ByteBuffer byteBuffer, final ReadableByteChannel channel) throws IOException {
    ...
    private static int readFromChannel(final ByteBuffer byteBuffer, final ReadableByteChannel channel)throws IOException{
        ByteBuffer header = getHeaderBuffer();
        channel.read(header);
        header.flip();
        int blockLen = header.getInt();
        //System.err.println(blockLen);
        ByteBuffer compressed = ByteBuffer.allocateDirect(blockLen);
        int rLen = channel.read(compressed);
        return getDecompressor().decompressDirectByteBuffer(byteBuffer, 0, byteBuffer.capacity(), compressed, 0, rLen);
    }

    private static long readLong(final ByteBuffer byteBuffer, final ReadableByteChannel channel) throws IOException {
        if (! byteBuffer.hasRemaining()) {
            byteBuffer.clear();
            final int result = readFromChannel(byteBuffer, channel);
            assert result != 0;
            if (result == -1) throw new EOFException();
    ...
    protected void finalize() throws Throwable {
        try {
            if (! closed) {
                LOGGER.warn("This " + this.getClass().getName() + " [" + this + "] should have been closed.");
                close();
            }
        }
        finally {
    ...
    public void check() throws DuplicateException {
        for(final Bucket b: this) b.iterator();
    }
    ...
    /** A bucket returned by a {@link ConcurrentBucketedHashStore}. */
